---
description: 在 Obsidian vault 中按指定模板新建一篇笔记 — 自动套用 cortex 模板 (concept / entity / domain / dashboard / question / source), 填充 frontmatter, 按命名规则计算路径, 重名报错不覆盖。
argument-hint: "<type> <title>"
allowed-tools: Bash, Read, Write, Glob, mcp__obsidian__obsidian_get_file_contents, mcp__obsidian__obsidian_append_content, mcp__obsidian__obsidian_list_files_in_dir
---
...
1. 解析 vault 路径 (同 `/cortex:install`)
...
- 不更新 `index.md` / `hot.md` (这是 cortex-save / lint 的职责)
