...
2. **判定目录类型** (按下表自决, 禁问):
   ...
   | 无 `.git`, 有 `pyproject.toml` / `package.json` / `Cargo.toml` / `go.mod` | **项目** | `知识库/项目/local/<basename>/...` |
   | 其余 (普通目录兜底) | **项目** | `知识库/项目/local/<basename>/_index.md` |
...
5. **落档** (强 folder-first; 详见 cortex-ingest SKILL §1 文件夹优先 + §3 强制 frontmatter):
   ...
   - 普通项目 (无 git) → `知识库/项目/local/<basename>/_index.md` + 子文档
