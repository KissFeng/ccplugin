...
5. **落档** (强 folder-first; 详见 cortex-ingest SKILL §1 文件夹优先 + §3 强制 frontmatter):
   ...
   - frontmatter 必填: type / title / desc / created / updated / tags (语义 tag, 严禁占位; 严禁 hierarchical 前缀 `type/x` / `source/x` 等, 这些走字段) / source_url / version / when_to_read / score (1-5) / maturity (详见 SKILL §3)
   - 元数据走 **frontmatter 字段** (host / org / repo / source_kind / domain / stack / lang / level), 不重复成 hierarchical tag (详见 SKILL §6 + ingest/references/global-rules.md §6)
