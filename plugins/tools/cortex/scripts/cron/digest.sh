...
PROMPT="[AUTO_MODE persistent] Invoke Skill cortex-digest on vault=$VAULT lang=${LANG_CODE:-zh-CN}. The skill SKILL.md is the single source of truth for the five-phase pipeline (read / analyze / process / update / cleanup). Follow it strictly, no skip, no ask. Emit the compact JSON described in the skill '## 输出' section."
