...
具体行动:
1. L0 完整性: 若 <vault>/.git 存在, 跑 \`git -C <vault> log --format=%H -- 记忆体系/L0-核心/\` + \`git -C <vault> tag --contains <commit>\` 检查 L0 改动是否有 git tag 锚定。无 tag 的 L0 改动 → 严重报警。
2. L1/L2 矛盾: 扫 <vault>/记忆体系/L1-长期/{procedural,semantic-stable}/* 与 L2-中期/semantic/* 的 frontmatter (尤其 ref/topic 字段) 与 brief。同 topic 多条目语义冲突 (AI 判读, 谨慎假阳) → 中度报警。
...
5. 写产物:
   ...
   b. <vault>/记忆体系/views/alerts.md 追加 ## memory-warden <UTC ISO> 一节, 仅记 severity=high 项。
...
exec "$DIR/run.sh" memory-warden -- "$PROMPT" \
