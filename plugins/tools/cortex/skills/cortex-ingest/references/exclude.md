...
| AI 工具配置 | `.claude/` `.claude-plugin/` `.codex/` `.opencode/` `.cursor/` `.continue/` `.windsurf/` `.trae/` `.aider*/` `.copilot/` `.sourcegraph/` `.tabnine/` `.sweep/` `.trellis/` `.cursorrules` `.cursorignore` `.windsurfrules` `.aider.conf.yml` |
| 临时 / 备份 | `*.{bak,swp,tmp,old,orig,backup,~}` `*.swp.*` `.vscode-test/` |
| 压缩包 | `*.{zip,tar,gz,tgz,xz,bz2,rar,7z}` |

> **注意**: `AGENTS.md` / `CLAUDE.md` / `.github/copilot-instructions.md` 等是项目级文档 (面向人类 + 多 AI 工具), 内容有价值, **不剔除** — 由 ingest 正常摄取。剔除的仅是工具私有状态目录 (会话 / 缓存 / 临时 prompt 文件)。
