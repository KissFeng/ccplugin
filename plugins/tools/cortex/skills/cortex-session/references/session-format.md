# cortex-session — 会话格式 + frontmatter 规范

> SKILL.md 入口的 transcript 解析 + 写盘格式细节。
...
`记忆/L4-流水账/sessions/<cli>/<YYYY-MM>/<sid>.md`
...
每个关键事件 (tool error / file write / user clarification) → `记忆/L4-流水账/ledger/<YYYY-MM-DD>.jsonl` 一行:
...
若目标 session md 已存且 `--since` 未指定 → 跳过, 标 `(skipped, exists)`。
