...
- **CLI 直接调 (12)**: `save.sh` / `search.sh` / `deep_search.sh` / `ingest_url.sh` / `ingest_file.sh` / `ingest_remote.sh` / `refresh_projects.sh` / `memory.sh` / `ledger.sh` / `session.sh` / `html_render.sh` / `image_gen.sh`
