...
```
知识库/
├── 项目/<host>/<org>/<repo>/         git repo + 本地项目 (host=local 时为 项目/local/<basename>/)
├── 来源/{网页,论文,书籍}/             外部输入 (非 repo)
├── 领域/                              7 大预设
...
**知识库子目录**:
- `知识库/项目/<host>/<org>/<repo>/` (git repo + 本地项目, host=local 时为 项目/local/<basename>/)
- `知识库/来源/{网页,论文,书籍}/` (仅非 repo 来源)
