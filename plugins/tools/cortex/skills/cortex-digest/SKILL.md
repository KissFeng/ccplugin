...
| 5 | 整合 (Consolidate) 双向 (new) | **5a** 项目→领域 (LLM 抽通用概念到 `知识库/领域/<域>/`); **5b** 记忆→知识库 (L1/L2 stable + weight≥0.7 + recall≥5 → 晋升领域); **5c** 知识库→记忆 (领域 score≥7 + backref≥3 → 具化 L1/L2 operator 记忆); **5d** 双向 backlink 同步 + `_meta/bridge.jsonl` 审计落痕 | 写 state/consolidate.json + `_meta/bridge.jsonl` |
...
| [consolidate.md](references/consolidate.md) | 阶段 5a 项目→领域 提炼算法 + LLM prompt + 合并冲突 |
| [memory-kb-bridge.md](references/memory-kb-bridge.md) | 阶段 5b/5c/5d 记忆 ↔ 知识库 双向桥 + backlink 同步 + `_meta/bridge.jsonl` 审计 |
