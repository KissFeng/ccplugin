---
name: cortex-reflect
description: DMN 跨域综合 — 读最近高频 L2/L3 找跨 domain 连接 + 未解决疑问 → 知识库/反思/。触发: "reflect" / "反思" / "周/月复盘" / weekly cron。
...
1. **读源**:
   - Glob `记忆体系/L2-中期/semantic/**/*.md` (weight ≥ 0.5)
   - Glob `记忆体系/L3-短期/episodic/<window>/*.md`
   - Glob `记忆体系/views/consolidated/<latest>.md` (cortex-consolidate 输出)
2. **domain 提取**:
   - 从 ref 字段反查 `知识库/领域/<L1>/<L2>/...` 推 domain 标签
   - 无 ref → 从 tags / 标题关键词推断
3. **连接发现** (kind=connection):
   - 不同 domain 实体在同一 episode/week 共现 ≥ 2 次
   - 写 `知识库/反思/连接/<YYYY-Wnn>.md`:
     ```markdown
...
4. **疑问归集** (kind=question):
   ...
   - 去重后归类到 domain
   - 写 `知识库/反思/疑问/<YYYY-Wnn>.md`
5. **洞察提议** (kind=insight, 可选):
   - 高 weight L2 条目 + 跨域引用 ≥ 3 → 候选洞察
   - 写 `知识库/反思/洞察/<YYYY-Wnn>.md` (草稿, 待用户精修)
...
[reflect] window=2026-W19
  ...
  connections: 4 (written to 知识库/反思/连接/2026-W19.md)
  questions: 11 (written to 知识库/反思/疑问/2026-W19.md)
  insights: 2 (draft to 知识库/反思/洞察/2026-W19.md)
```
