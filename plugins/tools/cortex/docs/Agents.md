...
**范围标记**: 📚 知识库 (vault) · 🧠 记忆层 · 🌐 外网 (WebFetch)
...
| `cortex-historian` | 📚 + 🧠 | 史官 — 多月份 sessions → folds 季度叙事 | cortex-session → cortex-summarizer → cortex-fold |
| `cortex-cartographer` | 📚 | 制图员 — canvas + dashboard 二件套 | cortex-canvas → cortex-dashboard → dashboard patch |
| `cortex-archivist` | 📚 | 档案员 — 收件箱/ 老化迁移提案 | cortex-search → cortex-refactor (move) |
