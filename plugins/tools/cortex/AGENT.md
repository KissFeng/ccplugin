...
cortex 全部能力以 **18 个 skill** (PR1-4 21→13, 后续增 cortex-config + cortex-image + cortex-image/video/audio-understand 至 18) 暴露, **19 个 slash command** (`/cortex:<name>` 触发 commands/<name>.md, 后者调用对应 skill)。
...
cortex 提供 **6 个专用 agent** (sub-agent, PR4 删 linker: 7→6), 在 18 个 skill 之上提供"接任务后多轮自主推进"能力。agent 与 skill 互补, 不替代。每个 agent 头部含 "vs <skill>" 分工注释行明确边界。
