...
cortex 共 16 个 skill, **~9 自动 / ~7 显式**。显式 skill frontmatter 标 `disable-model-invocation: true`, 不进 description 池, 主线模型不会自动激活, 必须用户明确请求 (中文短语 / 英文触发词 / slash 命令 / wrapper bash)。
...
bash wrappers 由 `scripts/install_wrappers.sh` 安装到 `~/.cortex/scripts/` (25 个: 10 slash + 2 shell + 13 CLI; image_understand 2026-05-22 新增), 内部 `exec python3 $PLUGIN_ROOT/scripts/cli/<name>.py "$@"`, 0 算法回退 (业务逻辑 100% 保留, 仅删 MCP 协议层)。`migrate.sh` 是一次性脚本, 不计入 24 集。
