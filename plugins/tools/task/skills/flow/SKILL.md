...
每次进入新阶段时，写入 `.KissFeng/tasks/{task_id}/.state.json`：
...
cat .KissFeng/tasks/{task_id}/align.json | grep '"user_confirmed": true'
```
...
import json, os
p = '.KissFeng/tasks/{task_id}/align.json'
if os.path.exists(p):
...
- context_file: `.KissFeng/tasks/{task_id}/context.json`
- task_align_file: `.KissFeng/tasks/{task_id}/align.json`
- task_file: `.KissFeng/tasks/{task_id}/task.json`
