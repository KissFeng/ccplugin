...
配置文件位置: ~/.KissFeng/ccplugin/notify/config.yaml
           或 .KissFeng/ccplugin/notify/config.yaml
"""
import os
import os.path
import shutil
...
def load_config() -> HooksConfig:
	...
	合并策略:
	1. 优先读取用户主目录 ~/.KissFeng/ccplugin/notify/config.yaml 作为基础配置
	2. 读取项目目录 .KissFeng/ccplugin/notify/config.yaml（如不存在则从 hooks.example.yaml 复制）
